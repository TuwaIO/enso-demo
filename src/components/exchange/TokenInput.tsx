  ...
  return (
    <div className="bg-[var(--tuwa-bg-tertiary)] p-4 rounded-xl border border-[var(--tuwa-border-primary)] hover:border-[var(--tuwa-border-secondary)] transition-colors group">
      <div className="flex justify-between items-center mb-2">
        <label className="text-xs font-semibold text-[var(--tuwa-text-secondary)] uppercase tracking-wider pl-1">{label}</label>
        {rightLabel}
      </div>
          ...
          {token ? (
            <>
              ...
              <div className="flex flex-col items-start min-w-0 flex-1">
                 <span className="text-sm font-bold text-[var(--tuwa-text-primary)] leading-tight truncate w-full text-left">{token.symbol}</span>
                 <span className="text-[10px] text-[var(--tuwa-text-secondary)] leading-tight truncate w-full text-left">{token.name}</span>
              </div>
            </>
          ) : (
